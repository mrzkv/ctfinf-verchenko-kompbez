...
## Идеи задач
Обобщим основные идеи (обобщение основано на множестве схожих задач прошлых годов).

1. В таблице ASCII каждому символу соответствует уникальный **байт**,
поэтому при разделение последовательности на группы мы разделяим их на байты. Либо как указано на рисунке/тексте/и т.д.
2. В задачах могут использоваться различные системы счисления.
3. Ответ в задаче это осмысленное слово. (Если с 1-го раза не получилось попробуйте переставить сопоставления объектов с цифраами)
4. Внимательно анализируйте текст задачи, в нем могут быть подсказки(например явные упоминания ASCII-кодов)